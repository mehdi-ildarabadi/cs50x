...
def index():
    ...
    # Getting the amount of cash the user
    ...
    # Updating the current price and in real time
    try:
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure username consists
        if not username.isalnum():
            return apology("Username must contain only characters and digits!", 200)

        # Ensure password is stronger
        if len(password) < 8:
            return apology("Password must be at least 8 characters long!", 200)
        if (
...
def sell():
    ...
    if request.method == "POST":
        ...
        cash = cash[0]["cash"]
        # Deposit value of sold
        current_price = stock["price"]
        cash += shares * current_price

        # Perform the sale
        for info in owned_stock:
            # Update database total amount he owns
